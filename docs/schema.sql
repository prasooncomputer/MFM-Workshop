...
CREATE TABLE IF NOT EXISTS product_process (
  ...
  depends_on_process_id INT NULL,
  FOREIGN KEY (product_id) REFERENCES products(id) ON DELETE CASCADE,
  FOREIGN KEY (process_id) REFERENCES processes(id),
  FOREIGN KEY (depends_on_process_id) REFERENCES processes(id),
  UNIQUE KEY uq_product_process (product_id, process_id)
);
...
CREATE TABLE IF NOT EXISTS orders (
  ...
  manual_locked TINYINT(1) DEFAULT 0,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
